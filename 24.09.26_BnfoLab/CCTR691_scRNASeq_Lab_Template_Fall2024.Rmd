...
> Red Flag 1: There were a reported 17,295 cells estimated in this sample

> Red Flag 2:The t-SNE Projection does not seem to accurately differentiate cell clusters
...
> Included metadata elements are: orig.ident; nCount_RNA; nFeature_RNA; active.assay
...
> **** There are 2757 samples. Seurat treats each cell as a "sample", and so there are 2757 cells. The 19. represents the number of genes being mapped to.
...
> 1314 cells survived the filtering.
...
> They have a similar distribution in terms of nFeature, however, it appears that a significant proportion of cells with an nCount near zero have been removed. Cells with high mitochondrial expression had outlier nFeature_RNA. Filtering helped clean the data by removing these cells. 
...
> X-axis = samples (Cells); Y-axis = genes contributing to the PC

Q10) How many cells are used in each heatmap?  

> 1314 (the number of cells remaining after filtering)

Q11) Describe what happens when the principal component (PC) number gets higher.

> Explained variance decreases and the contribution of specific genes to the overall variances become less distinct.
...
> Reducing the dimensions potentially loses important detail/clarity of clusters
...
> Much more prominent loss of detail, significant simplification of the plot
...
#Create a tSNE plot using the first 5 dimensions.
whim_filtered <- RunTSNE(whim_filtered, dims = 1:15)
DimPlot(whim_filtered, reduction = "tsne")

```

> Similar to the original 5 dimension plot clusters, but with potentially more detail and clearer clusters
...
> The average log2FC of SCGB2A2 = 2.799869
